        ...
        <div class="InfoWrap">
          ...
          <div class="choose">
            <div class="chooseArea">
              ...
              <dl v-for="(s, index) in spuSaleAttrList" :key="s.id">
                <dt class="title">{{ s.saleAttrName }}</dt>
                <dd
                  changepirce="0"
                  :class="{ active: sa.isChecked == 1 }"
                  v-for="(sa, index) in s.spuSaleAttrValueList"
                  :key="sa.id"
                  @click="ChangeActive(s.spuSaleAttrValueList, sa)"
                >
                  {{ sa.saleAttrValueName }}
                </dd>
              </dl>
            </div>
            <div class="cartWrap">
              ...
              <div class="add">
                <a href="javascript:" @click="SendInfoShopCart">加入购物车</a>
              </div>
            </div>
          </div>
        </div>
...
export default {
  ...
  mounted() {
    this.$store.dispatch("GetDetialList", this.$route.params.skuId);
  },
  methods: {
    ChangeActive(value, sa) {
      for (var i = 0; i < value.length; i++) {
        value[i].isChecked = 0;
      }
      sa.isChecked = 1;
    },
    SelectInputNum(event) {
